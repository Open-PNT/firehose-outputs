...
public:
	 ...
	 */
	Eigen::Matrix<uint8_t, Eigen::Dynamic, 1> get_image_data() const;

	/**
	 * Stores the encoded image. Interpretation varies based on the image_type.
	 */
	void set_image_data(Eigen::Matrix<uint8_t, Eigen::Dynamic, 1>);
	 ...
	 */
	Eigen::Matrix<double, Eigen::Dynamic, 1> get_model_coefficients() const;

	/**
	 * The camera model parameters, size depending on the distortion model.
	 */
	void set_model_coefficients(Eigen::Matrix<double, Eigen::Dynamic, 1>);
