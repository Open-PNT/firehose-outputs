...
MetadataMagneticField::MetadataMagneticField(TypeMetadataheader info,
	...
	this->c_struct             = aspn23_metadata_magnetic_field_new(
        info_prep, time_of_validity_prep, mounting_prep, sqrt(k.size()), k.data(), b.data());
...
std::vector<double> MetadataMagneticField::get_k() const {
	nullptr_check();
	if (c_struct->k == nullptr) return {};
	return {c_struct->k, c_struct->k + c_struct->num_meas * c_struct->num_meas};
}

void MetadataMagneticField::set_k(std::vector<double> k) {
	nullptr_check();
	memcpy(c_struct->k, k.data(), c_struct->num_meas * c_struct->num_meas * sizeof(double));

	c_struct->num_meas = sqrt(k.size());
}

std::vector<double> MetadataMagneticField::get_b() const {
	nullptr_check();
	if (c_struct->b == nullptr) return {};
	return {c_struct->b, c_struct->b + c_struct->num_meas};
}

void MetadataMagneticField::set_b(std::vector<double> b) {
	nullptr_check();
