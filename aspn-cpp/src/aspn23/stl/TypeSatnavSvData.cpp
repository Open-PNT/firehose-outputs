...
std::vector<double> TypeSatnavSvData::get_sv_pos() const {
	nullptr_check();

	return {&c_struct->sv_pos[0], &c_struct->sv_pos[0] + 3};
}

void TypeSatnavSvData::set_sv_pos(std::vector<double> sv_pos) {
	nullptr_check();
	memcpy(c_struct->sv_pos, sv_pos.data(), 3 * sizeof(double));
}

std::vector<double> TypeSatnavSvData::get_sv_vel() const {
	nullptr_check();

	return {&c_struct->sv_vel[0], &c_struct->sv_vel[0] + 3};
}

void TypeSatnavSvData::set_sv_vel(std::vector<double> sv_vel) {
	nullptr_check();
...
std::vector<float> TypeSatnavSvData::get_group_delay_vector() const {
	nullptr_check();

	return {&c_struct->group_delay_vector[0], &c_struct->group_delay_vector[0] + 4};
}

void TypeSatnavSvData::set_group_delay_vector(std::vector<float> group_delay_vector) {
	nullptr_check();
