...
std::vector<uint8_t> TypeImageFeature::get_descriptor() const {
	nullptr_check();
	if (c_struct->descriptor == nullptr) return {};
	return {c_struct->descriptor, c_struct->descriptor + c_struct->descriptor_size};
}

void TypeImageFeature::set_descriptor(std::vector<uint8_t> descriptor) {
	nullptr_check();
