...
TypeRemotePoint::TypeRemotePoint(
	...
	this->c_struct = aspn23_type_remote_point_new(included_terms,
	                                              id,
	                                              position_reference_frame,
	                                              position1,
	                                              position2,
	                                              position3,
	                                              sqrt(position_covariance.size()),
...
TypeRemotePoint::TypeRemotePoint(const TypeRemotePoint& other) {
	...
	this->c_struct = aspn23_type_remote_point_new(other.get_included_terms(),
	                                              ...
	                                              other.get_position2(),
	                                              other.get_position3(),
	                                              sqrt(other.get_position_covariance().size()),
...
std::vector<double> TypeRemotePoint::get_position_covariance() const {
	nullptr_check();
	if (c_struct->position_covariance == nullptr) return {};
	return {c_struct->position_covariance,
	        c_struct->position_covariance +
	            c_struct->num_position_components * c_struct->num_position_components};
}

void TypeRemotePoint::set_position_covariance(std::vector<double> position_covariance) {
	nullptr_check();
	memcpy(c_struct->position_covariance,
	       position_covariance.data(),
	       c_struct->num_position_components * c_struct->num_position_components * sizeof(double));

	c_struct->num_position_components = sqrt(position_covariance.size());
}
