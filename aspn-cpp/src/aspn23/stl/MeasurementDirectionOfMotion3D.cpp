...
MeasurementDirectionOfMotion3D::MeasurementDirectionOfMotion3D(
	...
	for (size_t row = 0; row < 2; row++)
		for (size_t col = 0; col < 2; col++)
			covariance_prep[row][col] = covariance[row * sqrt(covariance.size()) + col];
...
std::vector<double> MeasurementDirectionOfMotion3D::get_obs() const {
	nullptr_check();

	return {&c_struct->obs[0], &c_struct->obs[0] + 3};
}

void MeasurementDirectionOfMotion3D::set_obs(std::vector<double> obs) {
	nullptr_check();
	memcpy(c_struct->obs, obs.data(), 3 * sizeof(double));
}

std::vector<double> MeasurementDirectionOfMotion3D::get_error_vector() const {
	nullptr_check();

	return {&c_struct->error_vector[0], &c_struct->error_vector[0] + 3};
}

void MeasurementDirectionOfMotion3D::set_error_vector(std::vector<double> error_vector) {
	nullptr_check();
	memcpy(c_struct->error_vector, error_vector.data(), 3 * sizeof(double));
}

std::vector<double> MeasurementDirectionOfMotion3D::get_covariance() const {
	nullptr_check();

	return {&c_struct->covariance[0][0], &c_struct->covariance[0][0] + 2 * 2};
}

void MeasurementDirectionOfMotion3D::set_covariance(std::vector<double> covariance) {
	nullptr_check();
...
std::vector<double> MeasurementDirectionOfMotion3D::get_error_model_params() const {
	nullptr_check();
