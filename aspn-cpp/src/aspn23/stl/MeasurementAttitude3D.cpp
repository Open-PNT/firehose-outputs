...
MeasurementAttitude3D::MeasurementAttitude3D(
    ...
    std::vector<double> tilt_error_covariance,
	...
	for (size_t row = 0; row < 3; row++)
		for (size_t col = 0; col < 3; col++)
			tilt_error_covariance_prep[row][col] =
			    tilt_error_covariance[row * sqrt(tilt_error_covariance.size()) + col];
...
std::vector<double> MeasurementAttitude3D::get_quaternion() const {
	nullptr_check();

	return {&c_struct->quaternion[0], &c_struct->quaternion[0] + 4};
}

void MeasurementAttitude3D::set_quaternion(std::vector<double> quaternion) {
	nullptr_check();
	memcpy(c_struct->quaternion, quaternion.data(), 4 * sizeof(double));
}

std::vector<double> MeasurementAttitude3D::get_tilt_error_covariance() const {
	nullptr_check();

	return {&c_struct->tilt_error_covariance[0][0], &c_struct->tilt_error_covariance[0][0] + 3 * 3};
}

void MeasurementAttitude3D::set_tilt_error_covariance(std::vector<double> tilt_error_covariance) {
	nullptr_check();
...
std::vector<double> MeasurementAttitude3D::get_error_model_params() const {
	nullptr_check();
...
void MeasurementAttitude3D::set_error_model_params(std::vector<double> error_model_params) {
	nullptr_check();
