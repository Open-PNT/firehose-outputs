...
TypeMounting::TypeMounting(std::vector<double> lever_arm,
	...
	for (size_t row = 0; row < 3; row++)
		for (size_t col = 0; col < 3; col++)
			orientation_tilt_error_covariance_prep[row][col] = orientation_tilt_error_covariance
			    [row * sqrt(orientation_tilt_error_covariance.size()) + col];
...
std::vector<double> TypeMounting::get_lever_arm() const {
	nullptr_check();

	return {&c_struct->lever_arm[0], &c_struct->lever_arm[0] + 3};
}

void TypeMounting::set_lever_arm(std::vector<double> lever_arm) {
	nullptr_check();
	memcpy(c_struct->lever_arm, lever_arm.data(), 3 * sizeof(double));
}

std::vector<double> TypeMounting::get_lever_arm_sigma() const {
	nullptr_check();

	return {&c_struct->lever_arm_sigma[0], &c_struct->lever_arm_sigma[0] + 3};
}

void TypeMounting::set_lever_arm_sigma(std::vector<double> lever_arm_sigma) {
	nullptr_check();
	memcpy(c_struct->lever_arm_sigma, lever_arm_sigma.data(), 3 * sizeof(double));
}

std::vector<double> TypeMounting::get_orientation_quaternion() const {
	nullptr_check();

	return {&c_struct->orientation_quaternion[0], &c_struct->orientation_quaternion[0] + 4};
}

void TypeMounting::set_orientation_quaternion(std::vector<double> orientation_quaternion) {
	nullptr_check();
	memcpy(c_struct->orientation_quaternion, orientation_quaternion.data(), 4 * sizeof(double));
}

std::vector<double> TypeMounting::get_orientation_tilt_error_covariance() const {
	nullptr_check();

	return {&c_struct->orientation_tilt_error_covariance[0][0],
