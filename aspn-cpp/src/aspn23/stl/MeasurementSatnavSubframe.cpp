...
std::vector<int8_t> MeasurementSatnavSubframe::get_data_vector() const {
	nullptr_check();
	if (c_struct->data_vector == nullptr) return {};
	return {c_struct->data_vector, c_struct->data_vector + c_struct->num_bytes};
}

void MeasurementSatnavSubframe::set_data_vector(std::vector<int8_t> data_vector) {
	nullptr_check();
