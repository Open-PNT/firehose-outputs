...
bool aspn23_ros_interfaces__msg__measurement_direction_of_motion3_d__convert_from_py(
  ...
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("aspn23_ros_interfaces.msg._measurement_direction_of_"
                   "motion3_d.MeasurementDirectionOfMotion3D",
  ...
  { // error_vector
    PyObject *field = PyObject_GetAttrString(_pymsg, "error_vector");
    if (!field) {
      return false;
    }
    {
      ...
      assert(PyArray_TYPE(seq_field) == NPY_FLOAT64);
      Py_ssize_t size = 3;
      double *dest = ros_message->error_vector;
      for (Py_ssize_t i = 0; i < size; ++i) {
        double tmp = *(npy_float64 *)PyArray_GETPTR1(seq_field, i);
        memcpy(&dest[i], &tmp, sizeof(double));
      }
...
aspn23_ros_interfaces__msg__measurement_direction_of_motion3_d__convert_to_py(
    void *raw_ros_message) {
  /* NOTE(esteve): Call constructor of MeasurementDirectionOfMotion3D */
  PyObject *_pymessage = NULL;
  {
    PyObject *pymessage_module = PyImport_ImportModule(
        "aspn23_ros_interfaces.msg._measurement_direction_of_motion3_d");
    assert(pymessage_module);
    PyObject *pymessage_class = PyObject_GetAttrString(
        pymessage_module, "MeasurementDirectionOfMotion3D");
  ...
  { // error_vector
    PyObject *field = NULL;
    field = PyObject_GetAttrString(_pymessage, "error_vector");
    if (!field) {
      return NULL;
    }
    assert(field->ob_type != NULL);
    ...
    assert(sizeof(npy_float64) == sizeof(double));
    npy_float64 *dst = (npy_float64 *)PyArray_GETPTR1(seq_field, 0);
    double *src = &(ros_message->error_vector[0]);
    memcpy(dst, src, 3 * sizeof(double));
    Py_DECREF(field);
  }
  { // covariance
