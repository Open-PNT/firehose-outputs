...
bool aspn23_ros_interfaces__msg__type_header__convert_from_py(
  ...
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("aspn23_ros_interfaces.msg._type_header.TypeHeader",
                   full_classname_dest, 49) == 0);
  }
  aspn23_ros_interfaces__msg__TypeHeader *ros_message = _ros_message;
  { // vendor_id
  ...
  { // device_id
    ...
    assert(PyLong_Check(field));
    ros_message->device_id = PyLong_AsUnsignedLongLong(field);
    Py_DECREF(field);
  }
  { // context_id
    PyObject *field = PyObject_GetAttrString(_pymsg, "context_id");
    if (!field) {
      return false;
    }
    assert(PyLong_Check(field));
    ros_message->context_id = PyLong_AsUnsignedLong(field);
    Py_DECREF(field);
  }
  { // sequence_id
...
aspn23_ros_interfaces__msg__type_header__convert_to_py(void *raw_ros_message) {
  /* NOTE(esteve): Call constructor of TypeHeader */
  PyObject *_pymessage = NULL;
  {
    PyObject *pymessage_module =
        PyImport_ImportModule("aspn23_ros_interfaces.msg._type_header");
    assert(pymessage_module);
    PyObject *pymessage_class =
        PyObject_GetAttrString(pymessage_module, "TypeHeader");
  ...
  { // device_id
    PyObject *field = NULL;
    field = PyLong_FromUnsignedLongLong(ros_message->device_id);
    {
  ...
  { // context_id
    PyObject *field = NULL;
    field = PyLong_FromUnsignedLong(ros_message->context_id);
    {
      int rc = PyObject_SetAttrString(_pymessage, "context_id", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
