...
aspn23_measurement_attitude_2d_new(Aspn23TypeHeader* header,
	...
	Aspn23MeasurementAttitude2D* self =
	    (struct Aspn23MeasurementAttitude2D*)malloc(sizeof(Aspn23MeasurementAttitude2D));
	...
	if (attitude2d != NULL)
		memcpy(self->attitude2d, attitude2d, 2 * sizeof(double));
	else
		for (size_t ii = 0; ii < 2; ii++) self->attitude2d[ii] = NAN;
	...
	if (error_model_params != NULL && num_error_model_params != 0) {
		if (num_error_model_params == 0)
			...
			fprintf(
			    ...
			    "error_model_params");
			aspn23_measurement_attitude_2d_free(self);
			return NULL;
		}
	}
...
Aspn23MeasurementAttitude2D* ASPN_NULLABLE
aspn23_measurement_attitude_2d_copy(Aspn23MeasurementAttitude2D* input) {
	...
	Aspn23MeasurementAttitude2D* out =
	    aspn23_measurement_attitude_2d_new(header_prep,
...
void aspn23_measurement_attitude_2d_free(void* pointer) {
	Aspn23MeasurementAttitude2D* self = (Aspn23MeasurementAttitude2D*)pointer;
	if (NULL == self) return;
	aspn23_measurement_attitude_2d_free_members(self);
	free(self);
}
