...
aspn23_measurement_direction_3d_to_points_new(Aspn23TypeHeader* header,
	...
	Aspn23MeasurementDirection3DToPoints* self =
	    (struct Aspn23MeasurementDirection3DToPoints*)malloc(
	        sizeof(Aspn23MeasurementDirection3DToPoints));
	...
	if (obs != NULL && num_obs != 0) {
		if (num_obs == 0)
			...
			self->obs = (Aspn23TypeDirection3DToPoint*)calloc(num_obs,
			                                                  sizeof(Aspn23TypeDirection3DToPoint));
			for (size_t ii = 0; ii < num_obs; ii++) {
				Aspn23TypeDirection3DToPoint* pointer =
				    aspn23_type_direction_3d_to_point_copy(&obs[ii]);
				self->obs[ii] = *pointer;
				free(pointer);
			}
			...
			fprintf(
			    ...
			    "num_obs",
			    "obs");
			aspn23_measurement_direction_3d_to_points_free(self);
			return NULL;
		}
	}
...
Aspn23MeasurementDirection3DToPoints* ASPN_NULLABLE
aspn23_measurement_direction_3d_to_points_copy(Aspn23MeasurementDirection3DToPoints* input) {
	...
	Aspn23MeasurementDirection3DToPoints* out = aspn23_measurement_direction_3d_to_points_new(
...
void aspn23_measurement_direction_3d_to_points_free(void* pointer) {
	Aspn23MeasurementDirection3DToPoints* self = (Aspn23MeasurementDirection3DToPoints*)pointer;
	if (NULL == self) return;
	aspn23_measurement_direction_3d_to_points_free_members(self);
	free(self);
}
...
void aspn23_measurement_direction_3d_to_points_free_members(
	...
	if (self->obs != NULL && self->num_obs != 0) {
		for (size_t ii = 0; ii < self->num_obs; ii++)
			aspn23_type_direction_3d_to_point_free_members(&self->obs[ii]);
		free(self->obs);
	}
}
