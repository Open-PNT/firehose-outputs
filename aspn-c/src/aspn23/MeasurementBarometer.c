...
#include "MeasurementBarometer.h"
...
aspn23_measurement_barometer_new(Aspn23TypeHeader* header,
	...
	Aspn23MeasurementBarometer* self =
	    (struct Aspn23MeasurementBarometer*)malloc(sizeof(Aspn23MeasurementBarometer));
	...
	if (error_model_params != NULL && num_error_model_params != 0) {
		if (num_error_model_params == 0)
			...
			fprintf(
			    ...
			    "error_model_params");
			aspn23_measurement_barometer_free(self);
			return NULL;
		}
	}
...
Aspn23MeasurementBarometer* ASPN_NULLABLE
aspn23_measurement_barometer_copy(Aspn23MeasurementBarometer* input) {
	...
	Aspn23MeasurementBarometer* out =
	    aspn23_measurement_barometer_new(header_prep,
...
void aspn23_measurement_barometer_free(void* pointer) {
	Aspn23MeasurementBarometer* self = (Aspn23MeasurementBarometer*)pointer;
	if (NULL == self) return;
	aspn23_measurement_barometer_free_members(self);
	free(self);
}
