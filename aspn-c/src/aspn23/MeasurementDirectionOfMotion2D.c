...
#include "MeasurementDirectionOfMotion2D.h"
...
Aspn23MeasurementDirectionOfMotion2D* ASPN_NULLABLE aspn23_measurement_direction_of_motion_2d_new(
	...
	Aspn23MeasurementDirectionOfMotion2D* self =
	    (struct Aspn23MeasurementDirectionOfMotion2D*)malloc(
	        sizeof(Aspn23MeasurementDirectionOfMotion2D));
	...
	if (error_model_params != NULL && num_error_model_params != 0) {
		if (num_error_model_params == 0)
			...
			fprintf(
			    ...
			    "error_model_params");
			aspn23_measurement_direction_of_motion_2d_free(self);
			return NULL;
		}
	}
...
aspn23_measurement_direction_of_motion_2d_copy(Aspn23MeasurementDirectionOfMotion2D* input) {
	...
	Aspn23MeasurementDirectionOfMotion2D* out =
...
void aspn23_measurement_direction_of_motion_2d_free(void* pointer) {
	Aspn23MeasurementDirectionOfMotion2D* self = (Aspn23MeasurementDirectionOfMotion2D*)pointer;
	if (NULL == self) return;
	aspn23_measurement_direction_of_motion_2d_free_members(self);
	free(self);
}
