...
#include "MeasurementDeltaRange.h"
...
aspn23_measurement_delta_range_new(Aspn23TypeHeader* header,
	...
	Aspn23MeasurementDeltaRange* self =
	    (struct Aspn23MeasurementDeltaRange*)malloc(sizeof(Aspn23MeasurementDeltaRange));
	...
	if (error_model_params != NULL && num_error_model_params != 0) {
		if (num_error_model_params == 0)
			...
			fprintf(
			    ...
			    "error_model_params");
			aspn23_measurement_delta_range_free(self);
			return NULL;
		}
	}
...
Aspn23MeasurementDeltaRange* ASPN_NULLABLE
aspn23_measurement_delta_range_copy(Aspn23MeasurementDeltaRange* input) {
	...
	Aspn23MeasurementDeltaRange* out =
	    aspn23_measurement_delta_range_new(header_prep,
...
void aspn23_measurement_delta_range_free(void* pointer) {
	Aspn23MeasurementDeltaRange* self = (Aspn23MeasurementDeltaRange*)pointer;
	if (NULL == self) return;
	aspn23_measurement_delta_range_free_members(self);
	free(self);
}
