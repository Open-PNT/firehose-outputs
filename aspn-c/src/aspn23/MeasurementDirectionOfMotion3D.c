...
#include "MeasurementDirectionOfMotion3D.h"
...
Aspn23MeasurementDirectionOfMotion3D* ASPN_NULLABLE aspn23_measurement_direction_of_motion_3d_new(
	...
	Aspn23MeasurementDirectionOfMotion3D* self =
	    (struct Aspn23MeasurementDirectionOfMotion3D*)malloc(
	        sizeof(Aspn23MeasurementDirectionOfMotion3D));
	...
	if (obs != NULL)
		memcpy(self->obs, obs, 3 * sizeof(double));
	else
		for (size_t ii = 0; ii < 3; ii++) self->obs[ii] = NAN;

	if (error_vector != NULL)
		memcpy(self->error_vector, error_vector, 3 * sizeof(double));
	else
		for (size_t ii = 0; ii < 3; ii++) self->error_vector[ii] = NAN;
	...
	if (error_model_params != NULL && num_error_model_params != 0) {
		if (num_error_model_params == 0)
			...
			fprintf(
			    ...
			    "error_model_params");
			aspn23_measurement_direction_of_motion_3d_free(self);
			return NULL;
		}
	}
...
aspn23_measurement_direction_of_motion_3d_copy(Aspn23MeasurementDirectionOfMotion3D* input) {
	...
	Aspn23MeasurementDirectionOfMotion3D* out =
...
void aspn23_measurement_direction_of_motion_3d_free(void* pointer) {
	Aspn23MeasurementDirectionOfMotion3D* self = (Aspn23MeasurementDirectionOfMotion3D*)pointer;
	if (NULL == self) return;
	aspn23_measurement_direction_of_motion_3d_free_members(self);
	free(self);
}
