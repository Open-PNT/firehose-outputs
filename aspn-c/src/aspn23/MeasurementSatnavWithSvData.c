...
aspn23_measurement_satnav_with_sv_data_new(Aspn23TypeHeader* header,
	...
	Aspn23MeasurementSatnavWithSvData* self = (struct Aspn23MeasurementSatnavWithSvData*)malloc(
	    sizeof(Aspn23MeasurementSatnavWithSvData));
	...
	if (obs != NULL && num_signals_tracked != 0) {
		if (num_signals_tracked == 0)
			...
			fprintf(
			    ...
			    "num_signals_tracked",
			    "obs");
			aspn23_measurement_satnav_with_sv_data_free(self);
			return NULL;
		}
	}

	self->sv_data = NULL;
	if (sv_data != NULL && num_signals_tracked != 0) {
		if (num_signals_tracked == 0)
			self->sv_data = NULL;
		else if (num_signals_tracked > 0) {

			self->sv_data = (Aspn23TypeSatnavSvData*)calloc(num_signals_tracked,
			                                                sizeof(Aspn23TypeSatnavSvData));
			for (size_t ii = 0; ii < num_signals_tracked; ii++) {
				Aspn23TypeSatnavSvData* pointer = aspn23_type_satnav_sv_data_copy(&sv_data[ii]);
				self->sv_data[ii]               = *pointer;
				free(pointer);
			}
...
Aspn23MeasurementSatnavWithSvData* ASPN_NULLABLE
aspn23_measurement_satnav_with_sv_data_copy(Aspn23MeasurementSatnavWithSvData* input) {
	...
	Aspn23MeasurementSatnavWithSvData* out =
	    aspn23_measurement_satnav_with_sv_data_new(header_prep,
...
void aspn23_measurement_satnav_with_sv_data_free(void* pointer) {
	Aspn23MeasurementSatnavWithSvData* self = (Aspn23MeasurementSatnavWithSvData*)pointer;
	if (NULL == self) return;
	aspn23_measurement_satnav_with_sv_data_free_members(self);
	free(self);
}
...
void aspn23_measurement_satnav_with_sv_data_free_members(Aspn23MeasurementSatnavWithSvData* self) {
	...
	if (self->sv_data != NULL && self->num_signals_tracked != 0) {
		for (size_t ii = 0; ii < self->num_signals_tracked; ii++)
			aspn23_type_satnav_sv_data_free_members(&self->sv_data[ii]);
		free(self->sv_data);
	}
