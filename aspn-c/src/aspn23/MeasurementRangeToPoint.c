...
#include "MeasurementRangeToPoint.h"
...
aspn23_measurement_range_to_point_new(Aspn23TypeHeader* header,
	...
	Aspn23MeasurementRangeToPoint* self =
	    (struct Aspn23MeasurementRangeToPoint*)malloc(sizeof(Aspn23MeasurementRangeToPoint));
	...
	if (error_model_params != NULL && num_error_model_params != 0) {
		if (num_error_model_params == 0)
			...
			fprintf(
			    ...
			    "error_model_params");
			aspn23_measurement_range_to_point_free(self);
			return NULL;
		}
	}
...
Aspn23MeasurementRangeToPoint* ASPN_NULLABLE
aspn23_measurement_range_to_point_copy(Aspn23MeasurementRangeToPoint* input) {
	...
	Aspn23MeasurementRangeToPoint* out =
...
void aspn23_measurement_range_to_point_free(void* pointer) {
	Aspn23MeasurementRangeToPoint* self = (Aspn23MeasurementRangeToPoint*)pointer;
	if (NULL == self) return;
	aspn23_measurement_range_to_point_free_members(self);
	free(self);
}
