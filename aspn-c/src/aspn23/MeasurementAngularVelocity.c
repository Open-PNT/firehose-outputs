...
Aspn23MeasurementAngularVelocity* ASPN_NULLABLE aspn23_measurement_angular_velocity_new(
	...
	Aspn23MeasurementAngularVelocity* self =
	    (struct Aspn23MeasurementAngularVelocity*)malloc(sizeof(Aspn23MeasurementAngularVelocity));
	...
	if (meas != NULL)
		memcpy(self->meas, meas, 3 * sizeof(double));
	else
		for (size_t ii = 0; ii < 3; ii++) self->meas[ii] = NAN;

	if (covariance != NULL)
		memcpy(self->covariance, covariance, 9 * sizeof(double));
	else
		for (size_t ii = 0; ii < 3; ii++)
			for (size_t jj = 0; jj < 3; jj++) self->covariance[ii][jj] = NAN;
	...
	if (error_model_params != NULL && num_error_model_params != 0) {
		if (num_error_model_params == 0)
			...
			fprintf(
			    ...
			    "error_model_params");
			aspn23_measurement_angular_velocity_free(self);
			return NULL;
		}
	}
...
aspn23_measurement_angular_velocity_copy(Aspn23MeasurementAngularVelocity* input) {
	...
	Aspn23MeasurementAngularVelocity* out =
...
void aspn23_measurement_angular_velocity_free(void* pointer) {
	Aspn23MeasurementAngularVelocity* self = (Aspn23MeasurementAngularVelocity*)pointer;
	if (NULL == self) return;
	aspn23_measurement_angular_velocity_free_members(self);
	free(self);
}
