...
aspn23_measurement_heading_new(Aspn23TypeHeader* header,
	...
	Aspn23MeasurementHeading* self =
	    (struct Aspn23MeasurementHeading*)malloc(sizeof(Aspn23MeasurementHeading));
	...
	if (error_model_params != NULL && num_error_model_params != 0) {
		if (num_error_model_params == 0)
			...
			fprintf(
			    ...
			    "error_model_params");
			aspn23_measurement_heading_free(self);
			return NULL;
		}
	}
...
Aspn23MeasurementHeading* ASPN_NULLABLE
aspn23_measurement_heading_copy(Aspn23MeasurementHeading* input) {
	...
	Aspn23MeasurementHeading* out = aspn23_measurement_heading_new(header_prep,
...
void aspn23_measurement_heading_free(void* pointer) {
	Aspn23MeasurementHeading* self = (Aspn23MeasurementHeading*)pointer;
	if (NULL == self) return;
	aspn23_measurement_heading_free_members(self);
	free(self);
}
