...
aspn23_measurement_time_difference_new(Aspn23TypeHeader* header,
	...
	Aspn23MeasurementTimeDifference* self =
	    (struct Aspn23MeasurementTimeDifference*)malloc(sizeof(Aspn23MeasurementTimeDifference));
	...
	if (error_model_params != NULL && num_error_model_params != 0) {
		if (num_error_model_params == 0)
			...
			fprintf(
			    ...
			    "error_model_params");
			aspn23_measurement_time_difference_free(self);
			return NULL;
		}
	}
...
Aspn23MeasurementTimeDifference* ASPN_NULLABLE
aspn23_measurement_time_difference_copy(Aspn23MeasurementTimeDifference* input) {
	...
	Aspn23MeasurementTimeDifference* out =
...
void aspn23_measurement_time_difference_free(void* pointer) {
	Aspn23MeasurementTimeDifference* self = (Aspn23MeasurementTimeDifference*)pointer;
	if (NULL == self) return;
	aspn23_measurement_time_difference_free_members(self);
	free(self);
}
