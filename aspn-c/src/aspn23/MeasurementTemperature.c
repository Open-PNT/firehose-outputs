...
#include "MeasurementTemperature.h"
...
aspn23_measurement_temperature_new(Aspn23TypeHeader* header,
	...
	Aspn23MeasurementTemperature* self =
	    (struct Aspn23MeasurementTemperature*)malloc(sizeof(Aspn23MeasurementTemperature));
	...
	if (error_model_params != NULL && num_error_model_params != 0) {
		if (num_error_model_params == 0)
			...
			fprintf(
			    ...
			    "error_model_params");
			aspn23_measurement_temperature_free(self);
			return NULL;
		}
	}
...
Aspn23MeasurementTemperature* ASPN_NULLABLE
aspn23_measurement_temperature_copy(Aspn23MeasurementTemperature* input) {
	...
	Aspn23MeasurementTemperature* out =
	    aspn23_measurement_temperature_new(header_prep,
...
void aspn23_measurement_temperature_free(void* pointer) {
	Aspn23MeasurementTemperature* self = (Aspn23MeasurementTemperature*)pointer;
	if (NULL == self) return;
	aspn23_measurement_temperature_free_members(self);
	free(self);
}
