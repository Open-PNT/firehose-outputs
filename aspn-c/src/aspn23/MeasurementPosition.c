...
aspn23_measurement_position_new(Aspn23TypeHeader* header,
	...
	Aspn23MeasurementPosition* self =
	    (struct Aspn23MeasurementPosition*)malloc(sizeof(Aspn23MeasurementPosition));
	...
	if (covariance != NULL && num_meas != 0 && num_meas != 0) {
		...
		if (covariance_elements > 0) {
			...
			fprintf(stderr,
			        ...
			        "num_meas",
			        "covariance");
			aspn23_measurement_position_free(self);
			return NULL;
		}
	}
...
Aspn23MeasurementPosition* ASPN_NULLABLE
aspn23_measurement_position_copy(Aspn23MeasurementPosition* input) {
	...
	Aspn23MeasurementPosition* out = aspn23_measurement_position_new(header_prep,
...
void aspn23_measurement_position_free(void* pointer) {
	Aspn23MeasurementPosition* self = (Aspn23MeasurementPosition*)pointer;
	if (NULL == self) return;
	aspn23_measurement_position_free_members(self);
	free(self);
}
