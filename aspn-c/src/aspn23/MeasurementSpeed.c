...
aspn23_measurement_speed_new(Aspn23TypeHeader* header,
	...
	Aspn23MeasurementSpeed* self =
	    (struct Aspn23MeasurementSpeed*)malloc(sizeof(Aspn23MeasurementSpeed));
	...
	if (error_model_params != NULL && num_error_model_params != 0) {
		if (num_error_model_params == 0)
			...
			fprintf(
			    ...
			    "error_model_params");
			aspn23_measurement_speed_free(self);
			return NULL;
		}
	}
...
Aspn23MeasurementSpeed* ASPN_NULLABLE aspn23_measurement_speed_copy(Aspn23MeasurementSpeed* input) {
	...
	Aspn23MeasurementSpeed* out = aspn23_measurement_speed_new(header_prep,
...
void aspn23_measurement_speed_free(void* pointer) {
	Aspn23MeasurementSpeed* self = (Aspn23MeasurementSpeed*)pointer;
	if (NULL == self) return;
	aspn23_measurement_speed_free_members(self);
	free(self);
}
