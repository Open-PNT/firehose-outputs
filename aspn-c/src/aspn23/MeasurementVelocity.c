...
aspn23_measurement_velocity_new(Aspn23TypeHeader* header,
	...
	Aspn23MeasurementVelocity* self =
	    (struct Aspn23MeasurementVelocity*)malloc(sizeof(Aspn23MeasurementVelocity));
	...
	if (covariance != NULL && num_meas != 0 && num_meas != 0) {
		...
		if (covariance_elements > 0) {
			...
			fprintf(stderr,
			        ...
			        "num_meas",
			        "covariance");
			aspn23_measurement_velocity_free(self);
			return NULL;
		}
	}
...
Aspn23MeasurementVelocity* ASPN_NULLABLE
aspn23_measurement_velocity_copy(Aspn23MeasurementVelocity* input) {
	...
	Aspn23MeasurementVelocity* out = aspn23_measurement_velocity_new(header_prep,
...
void aspn23_measurement_velocity_free(void* pointer) {
	Aspn23MeasurementVelocity* self = (Aspn23MeasurementVelocity*)pointer;
	if (NULL == self) return;
	aspn23_measurement_velocity_free_members(self);
	free(self);
}
