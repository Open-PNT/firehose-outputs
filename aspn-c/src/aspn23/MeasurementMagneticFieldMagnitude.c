...
aspn23_measurement_magnetic_field_magnitude_new(
	...
	Aspn23MeasurementMagneticFieldMagnitude* self =
	    (struct Aspn23MeasurementMagneticFieldMagnitude*)malloc(
	        sizeof(Aspn23MeasurementMagneticFieldMagnitude));
	...
	if (error_model_params != NULL && num_error_model_params != 0) {
		if (num_error_model_params == 0)
			...
			fprintf(
			    ...
			    "error_model_params");
			aspn23_measurement_magnetic_field_magnitude_free(self);
			return NULL;
		}
	}
...
Aspn23MeasurementMagneticFieldMagnitude* ASPN_NULLABLE
aspn23_measurement_magnetic_field_magnitude_copy(Aspn23MeasurementMagneticFieldMagnitude* input) {
	...
	Aspn23MeasurementMagneticFieldMagnitude* out =
	    aspn23_measurement_magnetic_field_magnitude_new(header_prep,
	                                                    time_of_validity_prep,
	                                                    input->field_strength,
	                                                    input->variance,
	                                                    input->error_model,
...
void aspn23_measurement_magnetic_field_magnitude_free(void* pointer) {
	Aspn23MeasurementMagneticFieldMagnitude* self =
	    (Aspn23MeasurementMagneticFieldMagnitude*)pointer;
	if (NULL == self) return;
	aspn23_measurement_magnetic_field_magnitude_free_members(self);
	free(self);
}
