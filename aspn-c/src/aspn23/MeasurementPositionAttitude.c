...
Aspn23MeasurementPositionAttitude* ASPN_NULLABLE aspn23_measurement_position_attitude_new(
	...
	Aspn23MeasurementPositionAttitude* self = (struct Aspn23MeasurementPositionAttitude*)malloc(
	    sizeof(Aspn23MeasurementPositionAttitude));
	...
	if (covariance != NULL)
		memcpy(self->covariance, covariance, 36 * sizeof(double));
	else
		for (size_t ii = 0; ii < 6; ii++)
			for (size_t jj = 0; jj < 6; jj++) self->covariance[ii][jj] = NAN;
	...
	if (error_model_params != NULL && num_error_model_params != 0) {
		if (num_error_model_params == 0)
			...
			fprintf(
			    ...
			    "error_model_params");
			aspn23_measurement_position_attitude_free(self);
			return NULL;
		}
	}
...
aspn23_measurement_position_attitude_copy(Aspn23MeasurementPositionAttitude* input) {
	...
	Aspn23MeasurementPositionAttitude* out =
...
void aspn23_measurement_position_attitude_free(void* pointer) {
	Aspn23MeasurementPositionAttitude* self = (Aspn23MeasurementPositionAttitude*)pointer;
	if (NULL == self) return;
	aspn23_measurement_position_attitude_free_members(self);
	free(self);
}
