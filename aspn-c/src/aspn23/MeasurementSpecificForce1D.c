...
Aspn23MeasurementSpecificForce1D* ASPN_NULLABLE aspn23_measurement_specific_force_1d_new(
	...
	Aspn23MeasurementSpecificForce1D* self =
	    (struct Aspn23MeasurementSpecificForce1D*)malloc(sizeof(Aspn23MeasurementSpecificForce1D));
	...
	if (error_model_params != NULL && num_error_model_params != 0) {
		if (num_error_model_params == 0)
			...
			fprintf(
			    ...
			    "error_model_params");
			aspn23_measurement_specific_force_1d_free(self);
			return NULL;
		}
	}
...
aspn23_measurement_specific_force_1d_copy(Aspn23MeasurementSpecificForce1D* input) {
	...
	Aspn23MeasurementSpecificForce1D* out =
...
void aspn23_measurement_specific_force_1d_free(void* pointer) {
	Aspn23MeasurementSpecificForce1D* self = (Aspn23MeasurementSpecificForce1D*)pointer;
	if (NULL == self) return;
	aspn23_measurement_specific_force_1d_free_members(self);
	free(self);
}
