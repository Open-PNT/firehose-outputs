...
#include "MeasurementAccumulatedDistanceTraveled.h"
...
aspn23_measurement_accumulated_distance_traveled_new(
	...
	Aspn23MeasurementAccumulatedDistanceTraveled* self =
	    (struct Aspn23MeasurementAccumulatedDistanceTraveled*)malloc(
	        sizeof(Aspn23MeasurementAccumulatedDistanceTraveled));
	...
	if (error_model_params != NULL && num_error_model_params != 0) {
		if (num_error_model_params == 0)
			...
			fprintf(
			    ...
			    "error_model_params");
			aspn23_measurement_accumulated_distance_traveled_free(self);
			return NULL;
		}
	}
...
aspn23_measurement_accumulated_distance_traveled_copy(
    Aspn23MeasurementAccumulatedDistanceTraveled* input) {
	...
	Aspn23MeasurementAccumulatedDistanceTraveled* out =
...
void aspn23_measurement_accumulated_distance_traveled_free(void* pointer) {
	Aspn23MeasurementAccumulatedDistanceTraveled* self =
	    (Aspn23MeasurementAccumulatedDistanceTraveled*)pointer;
	if (NULL == self) return;
	aspn23_measurement_accumulated_distance_traveled_free_members(self);
	free(self);
}
