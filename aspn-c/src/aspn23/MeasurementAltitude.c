...
aspn23_measurement_altitude_new(Aspn23TypeHeader* header,
	...
	Aspn23MeasurementAltitude* self =
	    (struct Aspn23MeasurementAltitude*)malloc(sizeof(Aspn23MeasurementAltitude));
	...
	if (error_model_params != NULL && num_error_model_params != 0) {
		if (num_error_model_params == 0)
			...
			fprintf(
			    ...
			    "error_model_params");
			aspn23_measurement_altitude_free(self);
			return NULL;
		}
	}
...
Aspn23MeasurementAltitude* ASPN_NULLABLE
aspn23_measurement_altitude_copy(Aspn23MeasurementAltitude* input) {
	...
	Aspn23MeasurementAltitude* out = aspn23_measurement_altitude_new(header_prep,
...
void aspn23_measurement_altitude_free(void* pointer) {
	Aspn23MeasurementAltitude* self = (Aspn23MeasurementAltitude*)pointer;
	if (NULL == self) return;
	aspn23_measurement_altitude_free_members(self);
	free(self);
}
